...
public class OrderDomainServiceImpl implements OrderDomainService {
    ...
    private void setOrderProductInformation(Order order, Store store) {
        // 매장의 상품 목록을 Map으로 변환합니다. 상품을 키로 사용하고, 상품의 이름과 가격을 값으로 사용합니다.
        Map<Product, Product> storeProductMap = store.getProducts().stream()
        ...
        order.getItems().forEach(orderItem -> {
            ...
            if (storeProductMap.containsKey(currentProduct)) {
                Product matchedProduct = storeProductMap.get(currentProduct);
